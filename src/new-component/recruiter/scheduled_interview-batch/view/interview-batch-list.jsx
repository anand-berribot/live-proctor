...
import ListOfInterviewRow from '../list-of-interview-row';
...
import InterviewCollapsibleTable from 'src/pages/recruiter/history/interview-batch-table/interview-collapsible-table';
...
export default function InterviewBatchList({ }) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.VITE_REACT_APP_PROCTOR_API_URL}mastermind/recruiter-view/get-scheduled-interview-list?page=${table.page + 1}&page_size=${table.rowsPerPage}&search_value=${filters.name || ''}&start_date=${filters?.startDate || ''}&end_date=${filters.endDate || ''}&file_uploaded_id=${batchId}`, {
                method: 'GET',
                headers: {
    ...
    const getPercentByCompletion = (status) => (status / (completed + ongoing + scheduled)) * 100
    const getPercentByOngoing = (status) => (status / (completed + ongoing + scheduled)) * 100
    const getPercentByScheduled = (status) => (status / (completed + ongoing + scheduled)) * 100
    const f100 = (num) => (num / (completed + ongoing + scheduled)) * 100
    const tabs = [
    ...
    // console.log(ongoing, 'dataInPage')
    return (
        <>
            ...
            >
                <CustomBreadcrumbs
                    // heading="Scheduled Interview Batch List"
                    links={[{ name: 'Dashboard', href: '/dashboard' }, { name: 'History', href: '/dashboard/history' }, { name: 'Scheduled Interview Batch List' }]}
                />
                <Typography variant="h4" sx={{ my: 2 }}>{location?.state}</Typography>
                <Card
                    sx={{
                        mb: { xs: 3, md: 3 },
                    }}
                    ...
                    <Scrollbar>
                        ...
                        >
                            <InvoiceAnalytic
                                ...
                                // total={data?.total}
                                // total={100}
                                total={completed + ongoing + scheduled}
                                percent={f100(completed + ongoing + scheduled)}
                            ...
                            <InvoiceAnalytic
                                ...
                                percent={getPercentByCompletion(data?.data?.Completed)}
                            ...
                            <InvoiceAnalytic
                                ...
                                percent={getPercentByOngoing(data?.data?.Ongoing)}
                            ...
                            <InvoiceAnalytic
                                ...
                                // title="Un-attempted"
                                // total={getInvoiceLength('pending')}
                                percent={getPercentByScheduled(data?.data?.scheduled)}
